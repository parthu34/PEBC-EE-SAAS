...
export default function MockExam(){
  ...
  const [left,setLeft]=useState(45*60); const [attemptId,setAttemptId]=useState<string>("");
  ...
  useEffect(()=>{(async()=>{
    ...
    const r = await fetch("/api/attempt",{method:"POST",headers:{'Content-Type':'application/json'},body:JSON.stringify({mode:'mock'})});
    ...
    setLeft(remaining);
    const savedIndex = Number(localStorage.getItem("mock:lastIndex")||"0");
  ...
  const submit = useCallback(async()=>{
    ...
    localStorage.removeItem("mock:attemptId");
  ...
  useEffect(()=>{ if(attemptId) localStorage.setItem("mock:attemptId", attemptId); },[attemptId]);
  useEffect(()=>{ localStorage.setItem("mock:lastIndex", String(i)); },[i]);
  ...
  const lowTime = left <= Math.max(60, Math.floor(limit * 0.1));

  return (
    <main className="container">
      <div className="card">
        <div className="flex justify-between items-center mb-2">
          <div className="flex gap-2 items-center"><span className="badge">Free Mock (30Q)</span><span className="badge">{i+1}/{qs.length||30}</span><span className="badge">{q?.topic||""}</span></div>
          ...
          <>
            ...
            <div className="mt-3 flex justify-between">
              <div className="flex gap-2">
                ...
                <button className="btn" onClick={()=>setI(i=>Math.min((qs.length||30)-1,i+1))}>Next</button>
